    ...
    def apply_transpose(self, transpose_dict):
        ...
        for char in self.message_text:
            if str.isalpha(char) is True:
                message_text_encrypted = message_text_encrypted + transpose_dict.get(char)
            else:
...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        for perms in permutations:
            transpose_dict = SubMessage.build_transpose_dict(self, perms)
            decrypted_message = SubMessage.apply_transpose(self, transpose_dict)
            decrypted_list = decrypted_message.split(" ")    
            valid_word_count = 0
            for word in decrypted_list:
                if is_word(self.valid_words, word) is True:
                    valid_word_count += 1       
            if valid_word_count is len(decrypted_list):
